...
BLYNK_AUTH = "4sQ3SVQwSDYxdYsS4g3k-dtEg9jEv5bF"  # your token
...
# Save images here: /home/student2/Raspi/motion_photos
CAPTURE_DIR = "/home/student2/Raspi/motion_photos"
...
CAPTURE_COOLDOWN = 2.0   # seconds between photos
last_capture_time = 0.0  # updated whenever we take a picture

def capture_motion_photo(distance_cm):
    """Capture a JPEG image when motion is detected (with cooldown)."""
    global last_capture_time
    now = time.time()
    if now - last_capture_time < CAPTURE_COOLDOWN:
        return  # too soon since last image, skip

    last_capture_time = now

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"motion_{ts}_{int(distance_cm)}cm.jpg"
...
def is_dark():
    # if backwards, flip to: return not ldr.is_pressed
...
def set_led_baseline():
    """
    DISARMED  → light-sensitive white lamp (via LDR)
    ARMED     → LED off unless alarming
    Override  → LED forced ON white
    """
    ...
    if armed:
        # In ARMED mode, LED is normally off (only used for alerts)
        rgb.off()
    else:
        # In DISARMED mode, LED acts as a light-sensitive lamp
        if is_dark():
            rgb.color = (1, 1, 1)
        else:
            rgb.off()
                ...
                if armed:
                    if last_distance is not None:
                        ...
                        if delta >= DISTANCE_CHANGE_THRESHOLD_CM:
                            ...
                            beep_and_flash_red()
                            # Take a photo (max once every 2 seconds)
                            capture_motion_photo(distance)
